...
C     .. Scalar Arguments ..
      COMPLEX*16 E
      REAL*8 C,Z
      INTEGER IPAN,KVREL,LMAX, irmd
C     ..
C     .. Array Arguments ..
      COMPLEX*16 DLOGDP(0:LMAXD),FZ(IRMD,0:LMAXD),
     +     HAMF(IRMD,0:LMAXD),MASS(IRMD),PZ(IRMD,0:LMAXD)
      REAL*8 DRDI(IRMD),DROR(IRMD),R(IRMD),S(0:LMAXD),
     +     VM2Z(IRMD)
      INTEGER IRCUT(0:IPAND)
C     ..
C     .. Local Scalars ..
      COMPLEX*16 AM1,BM1,DFD0,DPD0,FIP0,FIP1,HAMF1,K1F,K1P,K2F,K2P,
     +     K3F,K3P,K4F,K4P,MASS1,PIP0,PIP1,VME,VMEFAC,VMETR1
      REAL*8 DROR1,DRSM1,DRSP1,S1,SM1,SP1,SRAFAC,ZOCSQ
      INTEGER IP,IR,IRC,IRE,IRS,IRSP1,J,K,L
C     ..
C     .. Local Arrays ..
      COMPLEX*16 A(-1:4),B(0:4),DFDI(-4:0),DPDI(-4:0)
C     ..
...
c     Next line changed by T.K.
            IF (KVREL.EQ.1 .AND. Z.GT. 0.4D0 ) THEN
c            IF (KVREL.EQ.1 .AND. Z.GT.0.0D0) THEN
