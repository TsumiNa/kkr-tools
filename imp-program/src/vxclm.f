     ...
     +     IMAXSH,IFUNM,THETAS,YR,WTYR,IJEND,WORK,LMSP,irm,irid)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
C     ..
C     .. Scalar Arguments ..
      INTEGER IJEND,KSHAPE,KTE,KXC,LMAX,NEND,NSPIN,NSTART,irm,irid
C     ..
C     .. Array Arguments ..
      REAL*8 DRDI(IRM,*),EXC(0:LPOTD,*),GSH(*),R(IRM,*),
     +     RHO2NS(IRM,LMPOTD,NATYPD,*),
     +     THETAS(IRID,NFUND,*),V(IRM,LMPOTD,*),
     +     WORK(IRM,LMPOTD),WTYR(IJD,*),YR(IJD,*)
...
C     ..
C     .. Local Arrays ..
      REAL*8 ER(IRMaxD,0:LPOTD),ESTOR(IRMaxD,LMPOTD),EXCIJ(IJD),
...
C     .. External Functions ..
      ...
      EXTERNAL DDOT,MAPBLOCK
c	external MYNODE,NUMNODES
C     ..
C     .. External Subroutines ..
      EXTERNAL DAXPY,GCOLX,SIMP3,SIMPK,VOSKO,VXCSPO
C     ..
...
c          IN = MYNODE()
          ...
          DO 170 IQ1 = 1,IN
            IQ = IQ + WLEN1(IQ1)/8
  170     CONTINUE
          WRITE (6,FMT=*) IRC1,IN,IQ,WLEN1(IN+1)
          DO 200 ISPIN = 1,NSPIN
            IPOT = NSPIN* (IATYP-1) + ISPIN
            DO 190 LM = 1,LMMAX
              DO 180 IR = 2,IRC1
                WORK(IR,LM) = V(IR,LM,IPOT)
  180         CONTINUE
              CALL GCOLX(WORK(IQ,LM),WLEN1,V(2,LM,IPOT))
  190       CONTINUE
  200     CONTINUE
ccc          CALL GDSUM(ESTOR,IRM*LMPOTD,WORK)
ccc          CALL GDSUM(ER,IRM* (LPOTD+1),WORK)
ccc          CALL GDSUM(VXCR,2*NSPIND,WORK)
        END IF
