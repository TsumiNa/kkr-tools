      SUBROUTINE VINTRELA(AMAT,BMAT,CMOM,IREF,LMAX,NATPER,NATREF,NSPIN,
...
c
c   This is changed for the case of more atoms per unit shell ....
c
c-----------------------------------------------------------------------
C     .. Parameters ..
...
C     ..
C     .. Local Arrays ..
      REAL*8 ACH(LMPOTD,NTREFD,2),VOUT(LMPOTD),
     +                 VOUT1(LMPOTD),SN(3)
C     ..
...
c this is new
         IF (KSHAPE .eq. 0) then
c
c NOTE : Lattice relaxations in the present version
c        work only in the full potential case. The 
c        spherical version is not implemented. However
c        this subroutine is expected to work also 
c        for the spherical case (never tested)
c
           IF (I1.LT.1) THEN
              DO 620 L = 0,LMAX
              DO 630 M = -L,L
...
c  *****************************
c  VOUT,VOUT1 refer always to curent I1 atom no need for
c  double index array 
c
