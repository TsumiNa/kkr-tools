      SUBROUTINE IRWSOL(EK,LMAX,FZ,HAMF,MASS,PZ,QZ,SZ,DROR,S,IPAN,
     $     IRCUT,irmd)
      Implicit None
c-----------------------------------------------------------------------
...
C     .. Scalar Arguments ..
      COMPLEX*16 EK
      INTEGER IPAN,LMAX, irmd
C     ..
C     .. Array Arguments ..
      COMPLEX*16 FZ(IRMD,0:LMAXD),HAMF(IRMD,0:LMAXD),MASS(IRMD),
     +     PZ(IRMD,0:LMAXD),QZ(IRMD,0:LMAXD),SZ(IRMD,0:LMAXD)
      REAL*8 DROR(IRMD),S(0:LMAXD)
      INTEGER IRCUT(0:IPAND)
C     ..
C     .. Local Scalars ..
      COMPLEX*16 HAMF1,K1F,K1P,K2F,K2P,K3F,K3P,K4F,K4P,MASS1,QIM0,
     +               QIM1,SIM0,SIM1
      REAL*8 DROR1,DRSM1,DRSP1,S1,SM1,SP1
      INTEGER IP,IR,IRE,IRS,IRWSK,K,L
C     ..
C     .. Local Arrays ..
      COMPLEX*16 DQDI(0:4),DSDI(0:4)
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC MAX
C     ..
c$$$      IRWSK = IRCUT(1)/30
c     Timo's test starts
c     Use Wronski to avoid the inaccuracies near origin
      IRWSK = IRCUT(1)/9
c     Do not use the Wronski relation
c      IRWSK = 1
c     Timo's test ends
c
      DO 60 L = 0,LMAX
