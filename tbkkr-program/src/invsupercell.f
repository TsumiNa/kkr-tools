...
c                  - Z(N,NL)*B(N)*D(N)
               ...
               CALL BACKCONVERT(NL,N+1,F,GIN)

               CALL ZGEMM('N','N',NDIM,NDIM,NDIM,CONE,C(1,1,N),NDIM,
     +              F(1,1),NDIM,CZERO,E(1,1),NDIM)

               CALL BACKCONVERT(N+1,N+1,F,GIN)
...
c     write (6,*) 'it calculates the element ','(',irow,',',n,')'
     ...
     +              E(1,1),NDIM,CZERO,G(1,1),NDIM)
               ...
               CALL CONVERT(NL,NL-1,G,GIN)

            ELSE

c     Z(I,J+1) * M(J+1,J)

               CALL BACKCONVERT(N,ICOL+1,E,GIN)
