...
C     .. Array Arguments ..
      ...
      DOUBLE PRECISION Z(*),MTFAC(*),VBC(*),RBASIS(3,*)
...
C     .. Scalar Arguments ..
     ...
     +       VCONST,ABASIS,BBASIS,CBASIS,RCUTZ,RCUTXY,EBOTSC,EUPSC,
     +       EIMSC,EBOTGP,EUPGP,EIMGP
...
c
      ...
      IF (TEST('semicor ')) THEN

      CALL IoInput('SEMICORE ',UIO,2,7,IER)
                       READ (UNIT=UIO,FMT=*) EBOTSC,
     +                                       EUPSC,
     +                                       EIMSC,
     +                                       NPNTSC,
     +                                       NPNTIM 
      ...
      IF (TEST('gapcont ')) THEN

      CALL IoInput('GAPCONT ',UIO,2,7,IER)
                       READ (UNIT=UIO,FMT=*) EBOTGP,
     +                                       EUPGP,
     +                                       EIMGP,
     +                                       NPNTGP,
     +                                       NPNTIMGP 
...
c
      ...
      IF(KXC.ge.0) WRITE (6,FMT=9100) TXC(KXC+1)
	IF(KXC.LT.0) WRITE(6,*) ' PW91-GGA'
...
c      READ (7,FMT='(A40)') I25
      ...
      write(6,*) 'I25="',I25,'"'
      write(6,2100) 
      WRITE(6,2040) IFULLD,ISPARD,ISLABD
      WRITE(6,2110)
...
C *********************************************Input-End ********
 ...
 1029 FORMAT((F4.0,I4,4x,4I1,3I4,F8.4,I4))
C ------------------------------------------------------------------------
 2001 FORMAT(/,' ATOM',I3/)
