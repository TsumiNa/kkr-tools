      SUBROUTINE ESPscB(DEN,DF,E,ESPV,IE,LMAX,ISPIN,NATYP,factor)
      implicit none
c-----------------------------------------------------------------------
c
...
c
c     calculate the semicore contribution of the single particle energies
c       l and spin dependent .
c     calculate the change of the semicore single particle energies with
c       the local densities of states.
c     recognize that the density of states is always complex (see sub-
c      routine denst) .
c     this includes an implicit energy integration
c      (see subroutine RHOLMB) .
...
c     semicore version: scales the  DOS by factor to renormalize the
c     semicore charge (one should have an integer number of electrons
c     in the semicore to ensure a good total energy).
c-----------------------------------------------------------------------
C     .. Parameters ..
...
c
     ...
     +         DIMAG(E*DEN(IE,L,I1,ISPIN)*DF)*factor
   10   CONTINUE
   20 CONTINUE
      END
