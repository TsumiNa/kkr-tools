...
c EOF slab.f
c ************************************************************************
      SUBROUTINE SLAYDIRECT(LPOT,VEC1,VEC2,ALAT,BR,RM2,NRMAX,NSHLR,NSR,
     &                      SUM)
      implicit none
c -----------------------------------------------------------------
...
c     .. array arguments ..
      ...
      integer nsr(ishld)
c
      double precision rm2(2,nmaxd),BR(3,3)
      double precision vec1(3),vec2(3)
      double precision sum(LM2POTD),sumt(lm2potd)
c---  > integer variables
c   
      INTEGER NSHLR,NRMAX
      INTEGER ILAY,I1,I2,IAT,IS,NTEST
      INTEGER L,lm,M,K,LPOT,n,lm1,n5
      INTEGER IB,I,J,NLAYER,lm2pot,N1,MAXN,IA,N2
      integer N1A,N2A,IQ,itot,ipar
c---  > real variables
c
      DOUBLE PRECISION PI,FPI,ESUM,ALAT,B,SQROOT
      DOUBLE PRECISION ZZZ,XYXY,CCIB,YYY,R,CC,ZZ,fact,CR,X0,Y0,R2
      double precision zoffx,zoffy,stest,rmax
...
c     test the convergence
 ...
 10         CONTINUE
         ...
         end do                 ! all atoms in plane (n=1,nrmax  loop)
