...
c    .. local scalars ..
      INTEGER N,LM,INFO,LM1,LM2,LM3,LAYDO,LAYUP,NPER,
     +        I1,IND1,IND2,LDI1,LDI2,LDI3,
     +        IND3,LL3,I,J,i2,ILAYER,ICLUSTER,ISTEP1,ISTEP2,
     +        ILT1,ILT2,IL1,IL2,IROW
c     .. data statements ..
      INTEGER ICHECK(NLAYERD,NLAYERD)
c
c     .. external subroutines ..
      EXTERNAL CINIT,ZCOPY,ZGEMM,ZGETRF,ZGETRS,CONVERT
c
c
...
c
     ...
     +          E(1,1),1)
           ...
           CALL CONVERT(NLAYERD,NLAYERD,E,GIN)
     ...
     +          NDIM,DMAT(1,1,N),NDIM,CZERO,F(1,1),NDIM)
           ...
           CALL BACKCONVERT(N+1,N+1,E,GIN)
...
c     this is the loop for element G_ij with i<j
        ...
        DO 50 IROW=(N-1),1,(-1)
           ...
           if (icheck(irow,n).eq.1) then
             ...
             CALL BACKCONVERT(IROW+1,N,E,GIN)
...
c     this is the loop for element G_ij with i>j
        ...
        DO 60 IROW=N+1,NLAYERD,1
           ...
           if (icheck(irow,n).eq.1) then
              ...
              CALL BACKCONVERT(IROW,IROW,F,GIN)
     ...
     +             E(1,1),NDIM,INFO)
              ...
              DO I=1,NDIM
                 DO J=1,NDIM
c                   F(I,J)=GDIOLD(I,J,IROW)-(DINVER(I,J,IROW)-E(I,J))
                    stop 'invslab'
                 ENDDO
              ENDDO
