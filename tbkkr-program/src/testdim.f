     ...
     &                    nref,M2,IRNS,ncls,nlayer,LATT)
       ...
       include 'inc.fi'
       include 'inc.cls'
       INTEGER nspin,naez,nemb,nz,natyp,lmax,irm,ins,insref,ncls
       INTEGER M2
...
c
     ...
     *   ' inc.fi to',nspin
     ...
     +       ') in inc.fi to',naez
      ...
      if(nemb.ne.nembd) then
        write(6,*) 'Please, change the parameter nembd (',nembd,
     +       ') in inc.fi to',nemb
        stop_mark=1
      endif
c      if(nz.ne.nzd .and. .not.opt('no test ')) then
c        write(6,*) 'Please, change the parameter nzd in',
c     *   ' inc.fi to',nz
c        stop_mark=1
c      endif
...
c      if(m2.gt.m2d) then
c        write(6,*) 'Please, change the parameter m2d in',
c     *   ' inc.fi to',m2
c        stop_mark=1
c      endif
