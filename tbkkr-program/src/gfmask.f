...
c
c Prepare the matric icouple which has 1 in all nn' (atom) blocks
c that we nead .
c
         DO I=1,NAEZ
            DO J=1,NAEZ
     ...
     +                 ((NSH1(II).EQ.J).AND.(NSH2(II).EQ.I))) THEN 
         ...
         END IF ! Conductivity calculation
c
...
c     loop over the element icheck(i,j) with fixed j and i>j
         ...
         if ((ie.eq.1).OR.(ie.eq.iegfout)) then
            write (6,*) 'this is the icheck matrix'
            do i=1,nlayer
               write (6,FMT='(20i3)') (icheck(i,j),j=1,nlayer)
            enddo
         endif
