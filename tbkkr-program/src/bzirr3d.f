...
C Modified on 20.01.2000 To use the 2d inversion!
     ...
     &                   rfctor,wtkp,volbz,rsymat,nsymat,ISYMINDEX,irr)
      implicit none
!===========================================================================
!  Input:
...
!==========================================================================
      ...
      PARAMETER (MAXK1=100,MAXK2=100,MAXK3=100,NSYMAXD=48)
!i/o
      integer nkp,nkxyz(3),ibk(0:MAXK1,0:MAXK2,0:MAXK3),kpoibz,nsymat
      INTEGER ISYMINDEX(*)
      double precision kp(3,*),wtkp(*),rfctor,volbz
...
c Check if we are in surface mode
     ...
     &     bravais(3,3).eq.0.d0) lsurf = .true. 
      ...
      if (lsurf) nkxyz(3) = 1
        ALAT = RFCTOR*2.d0*pi
c     write(6,*) 'bzirr3d:',nkp,kpoibz,rfctor,'lsurf:',lsurf
...
c
      do i = 1, 3
         do j = 1, 3
            gq(i,j)=recbv(i,j)/nkxyz(j)
            rq(i,j)=bravais(i,j)*nkxyz(j) !/alat
         enddo 
      enddo
!===========================================================================
      if(irr) then
...
c Mesh point in the unit cell found.
      ...
      do i=1,nkp
         ...
         volbz = volbz + wtkp(i)*dfloat(nsym)
      end do    
            write(6,*) 'bzirr3d:',nkp,kpoibz,rfctor,'lsurf:',lsurf
            write(6,*) (nkxyz(i),i=1,3)    
      return
      end
