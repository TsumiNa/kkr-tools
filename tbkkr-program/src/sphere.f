...
C     ..
C     .. ARRAYS IN COMMON ..
      REAL*8 RIJ(IJD1,*),W(N1),WTYR(IJD1,*),X(N1),YR(IJD1,*)
      REAL*8 Y(LMMAXD),WEI
C     ..
C     .. LOCAL SCALARS ..
...
C
c.....------------------------------------------------------------------
C     dimension cosx(IJD1),fai(IJD1)
...
C     ..
C
      open(89,file='lebedev',FORM='unformatted',status='old')
      rewind 89
      WRITE(6,*) ' FORT.89 in sphee'
      READ(89) IJEND
      WRITE(6,*) IJEND
      IF(IJEND.GT.IJD1) STOP 'SPHERE'
C
      DO 30 IJ=1,IJEND
      READ(89) R1,R2,R3,WEI,COSX(IJ),FAI(IJ)
c     WRITE(6,9005) IJEND,IJ,R1,R2,R3,WEI,COSX(IJ),FAI(IJ)
      RIJ(IJ,1)=R1
      RIJ(IJ,2)=R2
      RIJ(IJ,3)=R3
C
