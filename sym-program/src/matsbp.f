      SUBROUTINE MATSBP(CNK,LNK,JMINA,JMAXA,JMINB,JMAXB,NTXA,GSYM,GMN,
...
C     ..
        ...
        DO 70 NM = NT0B,NTXBA
          ...
          DO 20 J = IMINB,IMAXB
            ...
            DO 10 I = IMINA,IMAXA
              CI = CNK(I,NN)
              LAMA = LNK(I,NN)
              GSYM(NN,NM) = GSYM(NN,NM) + GMN(LAMA,LBMB)*CI*CJ
   10       CONTINUE
   20     CONTINUE
          GO TO 70
   30     CONTINUE
   ...
   50     CONTINUE
          ...
          DO 60 I = IMINA,IMAXA
            CI = CNK(I,NN)
            LAMA = LNK(I,NN)
            GSYM(NN,NM) = GSYM(NN,NM) + (GMN(LAMA,LBMB1)*CJ1+
     +                    GMN(LAMA,LBMB2)*CJ2)*CI
   60     CONTINUE
   70   CONTINUE
        GO TO 180
   80   CONTINUE
  ...
  110     CONTINUE
     ...
     +                  (GMN(LAMA,LBMB1)*CJ1+GMN(LAMA,LBMB2)*CJ2)
  120   CONTINUE
        GO TO 180
  130   CONTINUE
        I = IMINA
        ...
        DO 170 NM = NT0B,NTXBA
          ...
          DO 140 J = IMINB,IMAXB
            CJ = CNK(J,NM)*NUMBER/NCOL
            LBMB = LNK(J,NM)
            GSYM(NN,NM) = GSYM(NN,NM) + (GMN(LAMA1,LBMB)*CI1+
     +                    GMN(LAMA2,LBMB)*CI2)*CJ
  140     CONTINUE
          GO TO 170
  150     CONTINUE
          J = IMINB
  ...
  160     CONTINUE
          ...
          CJ2 = CNK(J,NM)*NUMBER/NCOL
          LBMB2 = LNK(J,NM)
          GSYM(NN,NM) = GSYM(NN,NM) + (GMN(LAMA1,LBMB1)*CI1+
     +                  GMN(LAMA2,LBMB1)*CI2)*CJ1 +
     +                  (GMN(LAMA1,LBMB2)*CI1+GMN(LAMA2,LBMB2)*CI2)*CJ2
  170   CONTINUE
  180 CONTINUE
      RETURN
      END
